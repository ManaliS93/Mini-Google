...
public class BackUpMaster {
	...
	static boolean flag=false,flagHB=false,stopper=true;
	static int msPort;
	static String msIP;
	...
	public static void init() throws IOException {
		...
		File folder = new File("/afs/cs.pitt.edu/usr0/mcs116/public/MIndex");
		listFilesForFolder(folder);
		Socket socket = null;
		String msg="getMaster"+"//"+myIp+"//"+myPort;
		NS_send(msg);
	...
	public static void UDP_send(byte b[],int length,InetAddress destAddr,int destPort){
		 DatagramSocket sock;
		try {
			sock = new DatagramSocket();
		
		 DatagramPacket message = new DatagramPacket(b, length, 
                 destAddr, destPort);
		 		sock.send(message);
	...
	public static void listFilesForFolder(final File folder) {
	    for (final File fileEntry : folder.listFiles()) {
	        if (!fileEntry.isDirectory()) {
	            ...
	            try {
	                java.io.FileReader fr = new java.io.FileReader(fileEntry);
	                ...
	                while( (line = br.readLine()) != null ) {
	                	if (line.equals(""))
	                		continue;
	                    makeMap = line.split("#");
	                    if(!map.containsKey(makeMap[1])) {
	                    	List<IndexStore> t1 = new ArrayList<IndexStore>();
	                    	t1.add(new IndexStore(makeMap[2], Integer.parseInt(makeMap[3])));
	                    	map.put(makeMap[1], t1);
	                    }
	                    else {
	                    	map.get(makeMap[1]).add(new IndexStore(makeMap[2], Integer.parseInt(makeMap[3])));
	                    }
	                }

	            } catch (FileNotFoundException e) {
	                System.out.println("File not found: " + fileEntry.toString());
	            } catch (IOException e) {
	            	System.out.println("Unable to read file: " + fileEntry.toString());
				}
	        }
	    }
	}
	
	public static void display(Map<String, List<IndexStore>> map) {
				...
				try {
					...
					if (str.substring(0, 5).equals("index")&&( !str.contains("@"))) {
						...
						case "alive":
							...
							flag=false;
							break;
						case "NS_reply":msIP=gotreq[1];
						msPort=Integer.parseInt(gotreq[2]);
	...
	public static synchronized void updater() throws Exception, UnsupportedEncodingException {
		File dir = new File("C:\\fall2016\\OS\\dir_p");
		String item;
	...
	public static void HelperSend(String msg, String ip, int port) {
		Socket socket = null;
		try {
	...
	public static synchronized void build_service() {
